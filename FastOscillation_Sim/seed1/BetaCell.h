...
const double hubMutant = .00; //specified number of hubs change here and will update for randomVars etc....
bool mutationInFollowers = true; //1 for mutation in followers and 0 for muation in hubs

//Defines a two dimension vecor to append Icoup and Itot
vector<double> current(cellNumber * 2);
...
char* Icoupinfo = new char;
char* Itotinfo = new char;
//
...
/*			double KRevInt; 
			...
			Itot = IbNSC0 + IKDr0 + IKto0 + IKATP0 + ITRPM0 + ICaL0 + INaK0 + INaCa0 + IPMCA0 + IKslow0 + ICRAN0 + Icoup + IChR2;
...
#pragma omp for
		...
		if ((t - tOld) > 100)
		{
			...
			ofstream outfileIcoup;
			outfileIcoup.open(Icoupinfo, ios::app);
			ofstream outfileItot;
			outfileItot.open(Itotinfo, ios::app);
			...
			for (int b = 0; b < 2 * cellNumber; b = b + 2)
			{
				outfileIcoup << current[b] << ' ';
				outfileItot << current[b + 1] << ' ';
			}
